...
void printarray(int** myptr,int length){
    printf("**********ARRAY PRINT*****\n");
...
void shuffle(int** ptr, unsigned int length){
    for(int i = length - 1; i >= 0; i--){
        int* temp = ptr[i];
        unsigned int randomnum = (unsigned int)(rand() % length);
        ptr[i] = ptr[randomnum];
        ptr[randomnum] = temp;
    }
}
void shuffley(int* ptr, unsigned int length){
    for(int i = length - 1; i >= 0; i--){
        int temp = ptr[i];
        unsigned int randomnum = (unsigned int)(rand() % length);
        ptr[i] = ptr[randomnum];
